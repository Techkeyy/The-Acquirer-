...
async function recordPayment(apiId, amountUSDC, note) {
  try {
    ...
    const nonce = await getVault()
      .runner
      .provider
      .getTransactionCount(
        await getVault().runner.getAddress(),
        "pending"
      );

    const tx = await getVault().pay(
      apiId, amountEth, note,
      { nonce: nonce }
    );
...
async function checkBudgetSufficient(amountUSDC) {
  try {
    const remainingBudget = await getVault().remainingBudget();
